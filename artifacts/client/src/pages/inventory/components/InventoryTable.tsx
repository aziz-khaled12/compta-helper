...
export function InventoryTable({ state }: { state: ReturnType<typeof useInventoryState> }) {
  ...
  return (
    <Card className="bg-card">
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <Dialog open={isItemOpen} onOpenChange={setIsItemOpen}>
          <DialogTrigger asChild>
            <Button size="sm" variant="outline"><Plus className="h-4 w-4 mr-2" /> Nouvel Article</Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader><DialogTitle>Créer une fiche article</DialogTitle></DialogHeader>
            <Form {...itemForm}>
              <form onSubmit={itemForm.handleSubmit(onSubmitItem)} className="space-y-4 pt-4">
                <FormField control={itemForm.control} name="name" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input placeholder="Ex: Acier Inox..." {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <FormField control={itemForm.control} name="unit" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><Input placeholder="Ex: kg, L, unité" {...field} /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                </div>
                <Button type="submit" className="w-full mt-4" disabled={createItem.isPending}>Créer</Button>
              </form>
            </Form>
          </DialogContent>
        </Dialog>
      ...
      <CardContent>
        {items.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground border border-dashed rounded-lg">Aucun article en stock</div>
        ) : (
