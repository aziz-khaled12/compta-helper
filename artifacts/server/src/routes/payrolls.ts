...
router.delete("/payrolls/:id", async (req, res): Promise<void> => {
  ...
  // Payroll has no company_id of its own — it belongs to the tenant through its
  // employee — so ownership is checked with a subquery rather than a column
  // comparison. An id-only delete would let any authenticated user remove
  // another tenant's payslip; a foreign id is a silent no-op.
  const companyId = await getActiveCompanyId(req.user?.id);
  if (companyId) {
    await db
      .delete(payrollsTable)
      .where(
        and(
          eq(payrollsTable.id, params.data.id),
          inArray(
            payrollsTable.employeeId,
            db
