...
export default function Payroll() {
  ...
  const { data: payrolls, isLoading } = useListPayrolls({ monthYear: selectedMonth });
  ...
  const handleGenerateAll = async () => {
    ...
    for (const emp of employees) {
      ...
      if (!exists) {
        await generatePayroll.mutateAsync({ data: { employeeId: emp.id, monthYear: selectedMonth } });
      }
    }
    ...
    <div className="space-y-8 pb-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        ...
        <div className="flex items-center gap-2 bg-card border rounded-md p-1 shadow-sm">
          ...
          <Input 
            ...
            className="border-none shadow-none focus-visible:ring-0 h-8 w-[150px]"
          />
        </div>
      </div>
      ...
      <Card className="bg-card">
        <CardHeader className="flex flex-row items-center justify-between pb-2 border-b">
          ...
          <Button onClick={handleGenerateAll} variant="secondary" size="sm" disabled={!employees?.length || generatePayroll.isPending}>
            ...
            <TableBody>
              {employees?.map(emp => {
                ...
                return (
                  <TableRow 
                    key={emp.id} 
                    className={payslip ? "cursor-pointer hover:bg-muted/50" : ""}
                    ...
                    {payslip ? (
                      <>
                        ...
                        <TableCell className="text-right text-destructive text-sm">
                          -{formatMoney(payslip.cnasDeduction + payslip.irgDeduction)}
                        </TableCell>
                        <TableCell className="text-right font-bold text-primary">{formatMoney(payslip.netToPay)}</TableCell>
                        <TableCell className="text-right">
                          <Button variant="ghost" size="icon" onClick={(e) => handleDelete(payslip.id, e)}>
                      ...
                      <TableCell colSpan={4} className="text-right">
                        <Button 
                          variant="outline" 
                          size="sm" 
                          onClick={(e) => { e.stopPropagation(); handleGenerate(emp.id); }}
              ...
              {employees?.length === 0 && (
                <TableRow>
                  <TableCell colSpan={5} className="h-24 text-center text-muted-foreground">
                    Aucun employé enregistré. Allez dans Personnel pour en ajouter.
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
                ...
                <Table>
                  ...
                  <TableBody>
                    <TableRow className="border-none hover:bg-transparent">
                      ...
                      <TableCell className="text-right">{formatMoney(selectedPayslip.baseSalary)}</TableCell>
                      <TableCell></TableCell>
                    </TableRow>
                    {selectedPayslip.experienceBonus > 0 && (
                      <TableRow className="border-none hover:bg-transparent">
                        <TableCell>Prime d'ancienneté</TableCell>
                        <TableCell className="text-right">{formatMoney(selectedPayslip.experienceBonus)}</TableCell>
                        <TableCell></TableCell>
                      </TableRow>
                    )}
                    <TableRow className="bg-muted/30 font-semibold hover:bg-muted/30">
                      <TableCell>SALAIRE BRUT</TableCell>
                      <TableCell className="text-right">{formatMoney(selectedPayslip.grossSalary)}</TableCell>
                      <TableCell></TableCell>
                    </TableRow>
                    ...
                    <TableRow className="border-none hover:bg-transparent">
                      ...
                      <TableCell></TableCell>
                      <TableCell className="text-right text-destructive">{formatMoney(selectedPayslip.cnasDeduction)}</TableCell>
                    </TableRow>
                    
                    <TableRow className="bg-muted/30 font-semibold hover:bg-muted/30">
                      <TableCell>BASE IMPOSABLE (IRG)</TableCell>
                      <TableCell className="text-right">{formatMoney(selectedPayslip.taxableBase)}</TableCell>
                      <TableCell></TableCell>
                    </TableRow>
                    ...
                    <TableRow className="border-none hover:bg-transparent">
                      ...
                      <TableCell></TableCell>
                      <TableCell className="text-right text-destructive">{formatMoney(selectedPayslip.irgDeduction)}</TableCell>
                    </TableRow>
                  </TableBody>
                </Table>
